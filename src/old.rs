...
mod tests {
    use super::*;
    use nomt::hasher::{BinaryHasher, Sha2Hasher};
    ...
    fn test_commit_is_not_leaking_new_data_to_existing_sessions() {
        ...
        let test_writes_number = 30_000;
        let mut larger_accesses = Vec::with_capacity(test_writes_number);

        for i in 0..test_writes_number {
            let key = i.to_be_bytes().to_vec();
            let key_path: KeyPath = sha2::Sha256::digest(&key).into();
            let value = i.to_be_bytes().to_vec();
            larger_accesses.push((key_path, nomt::KeyReadWrite::Write(Some(value))));
        }

        larger_accesses.sort_by(|(k1, _), (k2, _)| k1.cmp(k2));

        let target_root_hash = {
            let session = nomt
                .begin_session(SessionParams::default().witness_mode(WitnessMode::read_write()));
            let finished_session = session
                .finish(larger_accesses.clone())
                .expect("finish failed");
            finished_session.root()
        };

        let writer_session =
            nomt.begin_session(SessionParams::default().witness_mode(WitnessMode::read_write()));
        let accesses_2 = vec![(key_path, nomt::KeyReadWrite::Write(Some(value_2.clone())))];
        let writer_finished_session = writer_session.finish(accesses_2).expect("finish failed");

        let competing_sessions = 1000;
        let mut competing_handles = Vec::with_capacity(competing_sessions);
        for _ in 0..competing_sessions {
            let session = nomt
                .begin_session(SessionParams::default().witness_mode(WitnessMode::read_write()));
            let this_accesses = larger_accesses.clone();
            let handle = std::thread::spawn(move || {
                std::thread::sleep(std::time::Duration::from_millis(10));
                let finished_session = session.finish(this_accesses).expect("finish failed");
                assert_eq!(finished_session.root(), target_root_hash);
            });
            competing_handles.push(handle);
        }

        println!("Trying to commit {:?}", std::time::Instant::now());
        writer_finished_session.commit(&nomt).unwrap();
        println!("commited: {:?}", std::time::Instant::now());

        for handle in competing_handles {
